...
import { Inbox, SearchX, ChevronLeft, Search, X, ListFilter, MoreVertical } from 'lucide-react';
...
class LaporanSayaPageBase extends React.Component<RouterProps, State> {
  ...
  async componentDidMount() {
    const [laporanRes, lokasiRes, kategoriRes] = await Promise.allSettled([LaporanApi.getMyLaporan({ limit: 100 }), LokasiApi.getAll(), KategoriApi.getAll()]);
    ...
    return (
      <div className="min-h-screen bg-brand-bg flex flex-col lg:pl-14">
        ...
        <div className="w-full max-w-3xl mx-auto flex flex-col flex-1">
          ...
          <div className="flex items-center gap-3 px-4 pt-5 pb-3 lg:pt-8">
            <button onClick={() => this.props.navigate(-1)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-brand-surface-alt text-brand-muted hover:text-brand-text transition-colors flex-shrink-0">
              <ChevronLeft size={18} />
            </button>
            <div>
              <h1 className="text-brand-text font-bold text-lg lg:text-2xl leading-tight">Laporan Saya</h1>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center gap-3 py-16 text-center">
                ...
                <div>
                  ...
                  <p className="text-brand-muted text-xs mt-0.5">{hasAnyFilter ? 'Coba ubah filter atau kata kunci pencarian' : 'Laporan yang kamu buat akan muncul di sini'}</p>
                </div>
                {hasAnyFilter && (
